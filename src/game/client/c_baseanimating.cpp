	...
	for (int i = 0; i < hdr->numbones(); ++i) 
	{
		...
		else
		{
				 ...
				 r_jiggle_bones.GetBool() && m_isJiggleBonesEnabled )
			{
				...
				{
					// PiMoN: ConcatTransofrms_Aligned crashes the game after loading bonematrix.m_flMatVal[0]
					// so have to use the old ConcatTransofrms there; shouldn't be a propler I suppose? Still,
					// needs investigating
...
#endif
ConVar cl_threaded_bone_setup( "cl_threaded_bone_setup", "1", 0, "Enable parallel processing of bones" );
...
#if defined( PORTAL2 ) || defined( INFESTED ) || defined( CSTRIKE_DLL )
...
int C_BaseAnimating::DrawModel( int flags )
{
	...
	{
		debugoverlay->AddTextOverlay( GetAbsOrigin(), 0, "Model: %s", m_pStudioHdr->pszName() );
	}
...
void C_BaseAnimating::DoAnimationEvents( CStudioHdr *pStudioHdr )
{
	...
	if (bLooped)
	{
		for (int i = 0; i < (int)seqdesc.numevents; i++)
		{
			...
			else if ( pevent[i].event < EVENT_CLIENT ) //Adrian - Support the old event system
				continue;
...
void C_BaseAnimating::GetRagdollInitBoneArrays( matrix3x4_t *pDeltaBones0, matrix3x4_t *pDeltaBones1, matrix3x4_t *pCurrentBones, float boneDt )
{
	...
	{
		Plat_FastMemcpy( pCurrentBones, m_CachedBoneData.Base(), sizeof( matrix3x4_t ) * m_CachedBoneData.Count() );
	}
}
...
C_BaseAnimating *C_BaseAnimating::CreateRagdollCopy()
{
	...
	pRagdoll->SetRenderColorA( GetRenderColor().a );
...
bool C_BaseAnimating::InitAsClientRagdoll( const matrix3x4_t *pDeltaBones0, const matrix3x4_t *pDeltaBones1, const matrix3x4_t *pCurrentBonePosition, float boneDt, Vector vecForceOverride )
{
	CStudioHdr *hdr = GetModelPtr();
	...
	m_pRagdoll = CreateRagdoll( this, hdr, vecForceOverride, m_nForceBone, pDeltaBones0, pDeltaBones1, pCurrentBonePosition, boneDt, false );
...
{
	return InitAsClientRagdoll( pDeltaBones0, pDeltaBones1, pCurrentBonePosition, boneDt, m_vecForce );
}
...
void C_BaseAnimating::OnDataChanged( DataUpdateType_t updateType )
{
	...
	if ( m_pRagdoll && m_nRenderFX != kRenderFxRagdoll || m_nRenderFX != kRenderFxRagdoll && m_builtRagdoll )
	{
		ClearRagdoll();
	}
...
void C_BaseAnimating::GetToolRecordingState( KeyValues *msg )
{
	...
	CStudioHdr *hdr = GetModelPtr();
	matrix3x4_t *pBones = (matrix3x4_t*)stackalloc( ( hdr ? hdr->numbones() : 1 ) * sizeof(matrix3x4_t) );
	if ( hdr )
	{
